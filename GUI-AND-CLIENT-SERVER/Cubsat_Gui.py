...
entry_packet_time = EntryText("temps entre les packets in ms", frame_protocol_config, "Time between packets", TOP, 5,
                              15)
scroll_choice_protocol = scrollField("Protocol Choice", frame_protocol_config, TOP,
                                     ["SPI", "UART", "I2C"])  # choose a protocol
send_button = SendButton("Send", frame_protocol_config, entry_packet_number, entry_packet_time, config_server,
                         scroll_choice_protocol, BOTTOM)
...
entry_digital_channel = SpinboxClass("Number Of digital channels", frame_saleae_config, 2, 8, TOP, 2, 2)
...
scroll_choice_trigger = scrollField("Trigger Choice", frame_saleae_config, TOP, ["Rising Edge",
                                                                                 "Falling Edge"])  # choose trigger, ajouter plus tard les deux options manquantes

entry_capture_time_and_sample_number = SpinboxClass("", frame_saleae_config, 1, 100, TOP, 1, 1)
field_length_capture_to_edit = ButtonRadio("field_capture", frame_saleae_config, TOP,
                                           entry_capture_time_and_sample_number,
                                           ["Capture Time(seconds) ", "Sample Number"], [(0, 100), (4e6, 100e9)])




# send
send_saleae_conf = SendSaleaeButton("Send", frame_saleae_config, BOTTOM, entry_analog_channel, entry_digital_channel,
                                    entry_analog_sample_rate, entry_digital_sample_rate, scroll_choice_trigger,
                                    field_length_capture_to_edit)
