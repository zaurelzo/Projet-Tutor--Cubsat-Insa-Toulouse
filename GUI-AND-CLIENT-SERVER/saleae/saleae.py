...
# Taken from https://github.com/kevinvalk/python-saleae
...
class ConnectedDevice():
    ...
    def __str__(self):
        if self.active:
            return "<saleae.ConnectedDevice #{self.index} {self.type} {self.name} ({self.id:x}) **ACTIVE**>".format(
                self=self)
        else:
...
class Saleae():
    ...
    def set_sample_rate_by_minimum(self, digital_minimum=0, analog_minimum=0):
        ...
        if best_rate is None:
            raise self.ImpossibleSettings(
                "No sample rate for configuration. Try lowering rate or disabling channels (especially analog channels)")
    ...
    def set_active_channels(self, digital, analog):
        ...
        if digital_no <= 0 and analog_no <= 0:
            raise ImpossibleSettings(
                'Logic requires at least one activate channel (digital or analog) and none are given')
    ...
    def _export_data_analog_csv(self, column_headers=True, delimiter='comma', display_base='hex',
    ...
    def _export_data_digital_csv(self, column_headers=True, delimiter='comma', timestamp='time_stamp',
    ...
    def export_data(self, file_path_on_target_machine, digital_channels=None, analog_channels=None, time_span=None,
                    format='csv', **export_args):
        '''Export command:
			EXPORT_DATA2,
			<filename>,
        ...
        if (digital_channels is None) and (analog_channels is None):
            ...
            if digital_channels is not None and len(digital_channels) and analog_channels is not None and len(
                    analog_channels):
